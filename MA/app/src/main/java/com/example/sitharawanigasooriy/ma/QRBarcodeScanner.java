...
public class QRBarcodeScanner extends AppCompatActivity implements ZXingScannerView.ResultHandler{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT>=Build.VERSION_CODES.M){
            if (checkPermission()){
                Toast.makeText(QRBarcodeScanner.this,"Permission is granted", Toast.LENGTH_LONG).show();
            }
            else{
                requestPermission();
            }
    ...
    private boolean checkPermission(){
        return (ContextCompat.checkSelfPermission(QRBarcodeScanner.this, CAMERA)== PackageManager.PERMISSION_GRANTED);
    }
    ...
    public void displayAlertMessage(String message, DialogInterface.OnClickListener Listner){
        new AlertDialog.Builder(QRBarcodeScanner.this).setMessage(message).setPositiveButton("OK",Listner).setNegativeButton("Cancel",null).create().show();
    }
    ...
    public void handleResult(Result result) {
        ...
        builder.setNeutralButton("Visit", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Intent qrintent = new Intent(Intent.ACTION_VIEW, Uri.parse(scanResult));
